...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	...
	if err := ctx.Service(&freecoin); err == nil {
		apiBackend = freecoin.ApiBackend
	} else {
		var freecoin *les.LightEthereum
		if err := ctx.Service(&freecoin); err == nil {
			apiBackend = freecoin.ApiBackend
		} else {
			return nil, err
...
func (r *ReleaseService) checkVersion() {
	...
	if version.Major > r.config.Major ||
		...
		howtofix := fmt.Sprintf("Please check https://github.com/freecoin/free-go/releases for new releases")
