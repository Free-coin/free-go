...
var nodeDockerfile = `
FROM freecoin/client-go:latest
...
RUN \
  echo 'geth --cache 512 init /genesis.json' > geth.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.freecoin/keystore/ && cp /signer.json /root/.freecoin/keystore/' >> geth.sh && \{{end}}
...
services:
  {{.Type}}:
    ...
    volumes:
      - {{.Datadir}}:/root/.freecoin{{if .Ethashdir}}
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.freecoin"],
