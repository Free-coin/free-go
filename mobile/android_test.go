...
import (
	...
	"github.com/freecoin/free-go/internal/build"
)
...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.freecoin", new NodeConfig());
			node.start();
	...
	// signatures too. Regression test for free-go issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/freecoin/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.freecoin", "github.com/freecoin/free-go/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "freecoin", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
